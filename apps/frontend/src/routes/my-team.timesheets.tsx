...
function TimesheetsRoute() {
  ...
  // Wait for the session to restore before evaluating permissions — otherwise a
  // direct URL load (browser refresh) sees an empty permission set while auth
  // is still initialising and wrongly bounces to the dashboard.
  if (authStatus === "loading") return null;
  if (!isDhanshree && !hasPermission("my-team.timesheet-approval.view")) return <Navigate to="/" />;
  return (
    ...
    return [
      {
        ...
        taskId: projectsList[0]?.tasks[0]?.id || "",
        hours: [0, 0, 0, 0, 0, 0, 0],
        notes: ["", "", "", "", "", "", ""],
      },
    ];
      ...
      r.notes.forEach((n, idx) => {
        if (n.trim())
          cellComments[idx] = {
            status: "new",
            history: [
              { author: user.name, text: n, type: "comment", createdAt: new Date().toISOString() },
            ],
          };
      });
      return {
    ...
    <>
      {storeTs && storeTs.comments.length > 0 && (
        <div
          className={cn(
            ...
            storeTs.status === "rejected"
              ? "bg-red-50 border-red-200 text-red-800"
              : storeTs.status === "approved"
                ? "bg-green-50 border-green-200 text-green-800"
                : "bg-blue-50 border-blue-200 text-blue-800",
          )}
        >
          <div className="flex items-center gap-2 font-semibold mb-2">
            <AlertCircle className="h-4 w-4" /> Approver History & Comments
          </div>
          <div className="space-y-1.5 pl-6 text-xs max-h-32 overflow-y-auto">
            {storeTs.history.map((h, i) => (
              <p key={i}>
                <strong>{h.updatedBy}</strong> changed status to{" "}
                <span className="underline font-semibold">{h.status}</span> on{" "}
                {new Date(h.at).toLocaleString()}: <em>"{h.comment}"</em>
              </p>
            ))}
          </div>
        </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 rounded-xl border border-border bg-card p-4 shadow-sm">
        ...
        <div className="ml-auto flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={() => toast.success("Timesheet saved as draft!")}
          ...
          <button
            ...
            disabled={total === 0 || currentStatus === "approved"}
      ...
      {commentOpen && activeRow && (
        <div
          className="fixed inset-0 z-40 flex items-end justify-end bg-black/30 sm:items-center sm:justify-center"
...
function TimesheetApprovalView() {
  ...
  const selectedTs = useMemo(
    () => store.timesheets.find((t) => t.id === selectedTsId),
  ...
  const handleAction = (
    ...
    toast.success(
      `Timesheet marked as ${isChangeRequest ? "Change Requested" : status} persistently!`,
    );
    setActionComment("");
  ...
  return (
    <section className="rounded-xl border border-border bg-card shadow-sm">
          ...
          <tbody className="divide-y divide-border">
            {store.timesheets.map((t) => {
              ...
              const linkedProjects = Array.from(
                new Set(
                  t.entries
                    .map((e) => allProjects().find((p) => p.id === e.projectId)?.name)
                    .filter(Boolean),
                ),
              ).join(", ");
              return (
      ...
      {selectedTs && (
        <Modal
          title={`Review Timesheet — ${getPerson(selectedTs.userId)?.name}`}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-muted/20 border border-border rounded-lg p-3 text-xs leading-relaxed">
              ...
              <div>
                ...
                <p className="font-medium capitalize">{selectedTs.userRole}</p>
              </div>
              <div>
                            ...
                            return (
                              ...
                              >
                                ...
                                {hasComment && (
                                  <span
                                    className={cn(
                                      "ml-1 font-bold inline-block select-none",
                                      dotColor,
                                    )}
            ...
            <div className="flex justify-end gap-2 border-t border-border pt-3">
              ...
              <button
                onClick={() => handleAction("rejected", actionComment, true)}
                disabled={!actionComment.trim()}
              ...
              <div className="w-full max-w-lg rounded-xl border border-border bg-card p-5 shadow-2xl flex flex-col max-h-[90vh]">
                <div className="flex items-center justify-between border-b border-border pb-3">
                  <div>
                    <h3 className="text-sm font-semibold text-gray-900">
                      Day Comment Conversation
                    </h3>
                    <p className="text-[11px] text-muted-foreground">
                      Review and respond to daily timesheet comments
                    </p>
                  </div>
                  <button
                    onClick={() => {
                ...
                <div className="grid grid-cols-2 gap-2 bg-muted/20 border border-border rounded-lg p-3 my-3 text-xs leading-relaxed">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-700 truncate max-w-[200px]">{proj?.name}</p>
                  </div>
                  <div>
                    <span className="text-muted-foreground text-[10px] uppercase font-semibold">
                      Hours
                    ...
                    <p className="font-bold text-sm text-primary">
                      {entry.hours[activeCell.dayIndex]}h
                    </p>
                  </div>
                  <div className="col-span-2">
                    ...
                    <p className="font-medium text-gray-700 truncate">{tsk?.title}</p>
                  </div>
                </div>
                <div className="flex-1 overflow-y-auto min-h-[150px] pr-1 py-1 space-y-3">
                  {commentData && commentData.history.length > 0 ? (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="mt-1.5 text-[10px] text-muted-foreground border-t border-border/20 pt-1">
                            <p>
                              <strong>Created Date:</strong> {dt.toLocaleDateString()}
                            </p>
                            <p>
                    ...
                    <p className="text-center text-xs text-muted-foreground py-8">
                      No comments recorded for this entry.
                    </p>
                  )}
                </div>
